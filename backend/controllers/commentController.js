...
module.exports.createCommentforRepair = async (req, res) => {
    try {
        ...
        const comment = await Comments.create(req.body)
        // find the post 
        await Repair.findByIdAndUpdate(req.params.pid, {
            // and push the new comment document's id
            $push: {
                // to the post's comments field/property
                comments: comment._id
...
//for repair
module.exports.deleteCommentforRepair = async (req, res) => {
    try {
        // first use the id to delete the comment from the comments collection
        await Comments.findByIdAndDelete(req.params.id)
        // then use the post's id to find the post
...
//for repair
module.exports.indexCommentforRepair = async (req, res) => {
    try {
        // target the comments property 
        const repair = await Repair.findById(req.params.pid).populate('comments')
        res.json(repair.comments)
    } catch(err) {
        res.status(400).json({ error: err.message })
    }
}
